...
public protocol UNCropperViewControllerDelegate: AnyObject {
    func cropperDidConfirm(_ cropper: UNCropperViewController, state: CropperState?)
    func cropperDidCancel(_ cropper: UNCropperViewController)
}

public extension UNCropperViewControllerDelegate {
    func cropperDidCancel(_ cropper: UNCropperViewController) {
        cropper.dismiss(animated: true, completion: nil)
    }
}

open class UNCropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    public weak var delegate: UNCropperViewControllerDelegate?
    ...
    public var verticalAspectRatios: [AspectRatio] = []
    ...
    public func commitCroppedImage() {
        delegate?.cropperDidConfirm(self, state: saveState())
    }
    
    public func angleRulerValueChanged(with value: CGFloat) {
        scrollViewContainer.isUserInteractionEnabled = false
        setStraightenAngle(value * CGFloat.pi / 180.0)
    }
    
    public func resetCropAction() {
        overlay.blur = false
    ...
    @objc func resetButtonPressed(_: UIButton) {
        resetCropAction()
    }
...
// MARK: - Private Methods
    ...
    open func resetToDefaultLayout() {
        ...
        let toolbarHeight = view.safeAreaInsets.bottom
        let bottomHeight = toolbarHeight
    ...
    public func matchScrollViewAndCropView(animated: Bool = false,
        ...
        if targetCropBoxFrame.equalTo(UNCropperViewController.overlayCropBoxFramePlaceholder) {
...
extension UNCropperViewController: UIScrollViewDelegate {
...
extension UNCropperViewController: UIGestureRecognizerDelegate {
...
extension UNCropperViewController: Stasisable, AngleAssist, CropBoxEdgeDraggable, AspectRatioSettable {}
